...
const char* topic = "Nicsap";  // MQTT topic to subscribe to
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  strncpy(cipherHex, (char*)payload, length);
  cipherHex[length] = '\0';  // Null-terminate the cipherHex string
  ...
  if (result == 0) {
    Serial.print("Decryption succeeded. Decrypted message: ");
